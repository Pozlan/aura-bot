...
async def dice_cmd(message: Message):
    ...
    if len(parts) < 2:
        await message.reply("usage: /dice &lt;amount&gt;  e.g. /dice 250k")
        return
    try:
    ...
    async with get_session() as session:
        ...
        if wager > available_balance(state):
            ...
            return
        try:
            challenge = await create_challenge(session, message.chat.id, "dice", user.id, wager)
        except InsufficientBalance:
            await message.reply("you don't have that much available.")
            return
        challenge_id = challenge.id

    house_available = not ECONOMY.DICE_MAX_HOUSE_WAGER or wager <= ECONOMY.DICE_MAX_HOUSE_WAGER
    framing = wager_framing(wager, ECONOMY.DICE_MAX_HOUSE_WAGER)
    text = (
        f"🎲 <b>Dice Duel · {format_amount(wager)}</b>\n\n"
        f"{pe('play')} <b>{esc(message.from_user.full_name)} wants to roll</b>\n"
