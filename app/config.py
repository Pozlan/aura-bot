...
class EconomyConfig:
    ...
    WORK_JOBS: dict = field(default_factory=lambda: {
        "cleaner": (8, 30),
        "delivery driver": (10, 38),
        "freelancer": (13, 60),
        "mechanic": (15, 45),
        "developer": (20, 80),
        "chef": (13, 43),
        "driver": (10, 35),
        "security guard": (10, 33),
        "trader": (5, 100),
        "construction worker": (15, 48),
    })
    ...
    HUNT_MIN_STAKE: int = 1
    HUNT_MAX_STAKE: int = 250
    ...
    LUCK_MEDIUM_MIN: int = 1
    LUCK_MEDIUM_MAX: int = 50
    LUCK_BIG_MIN: int = 50
    LUCK_BIG_MAX: int = 100

    # House wager caps (0 = no cap -- unlimited wager allowed vs house)
    RPS_MAX_HOUSE_WAGER: int = 250
    COIN_MAX_HOUSE_WAGER: int = 250
    DICE_MAX_HOUSE_WAGER: int = 250
    # HighLow (solo one-shot game vs house)
    HIGHLOW_MAX_HOUSE_WAGER: int = 250
    HIGHLOW_MAX_ROUNDS: int = 15      # unused now that HighLow is one-shot, kept in case a streak mode returns
    DART_MAX_WAGER: int = 5_000
    BJ_MAX_HOUSE_WAGER: int = 250
    SLOTS_MAX_WAGER: int = 250
    ...
    ROBBERY_MIN_TARGET_BALANCE: int = 5
